...
export default function InboxPanel({ incomingJobs = [] }: InboxPanelProps) {
  const [activeTab, setActiveTab] = useState<TabId>('new');
  ...
  useEffect(() => {
    if (incomingJobs.length > 0) {
      ...
      const lastJob = incomingJobs[0];
      
      // הפיכת הנתון מה-API לפורמט של ה-Inbox שלנו
      const newEntry: InboxEntry = {
        id: lastJob.messageId || Date.now(), // מזהה ייחודי
        subject: "פנייה חדשה מהמערכת", // אפשר לקחת מה-category
        client: lastJob.senderName || "לקוח חדש",
        clientInitials: (lastJob.senderName || "ל").substring(0, 2),
        date: "היום",
        preview: lastJob.content || lastJob.text,
        budget: "₪ מחכה להצעה",
        status: 'בביצוע' // או כל סטטוס התחלתי אחר
      };
  ...
  }, [incomingJobs]); // רץ בכל פעם שמשימה חדשה נכנסת למערך
        ...
        <div className="flex gap-1 bg-gray-800 rounded-xl p-1">
          {tabs.map(tab => (
            ...
            >
              {tab.label}
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-white/20' : 'bg-gray-700'}`}>
                {/* הצגת כמות ההודעות מה-State המקומי */}
                {localInboxData[tab.id].length}
              </span>
            </button>
          ))}
        </div>
