...
export default function ProDashboard() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        // שליפת הזמנות שממתינות לאישור עבור בעל המקצוע הנוכחי
        //const response = await api.get(`/Requests/professional/${profId}/pending`);
        // זה יביא לחיים החשמלאי את כל הבקשות ששלחו אליו אישית והוא עוד לא אישר
       // מביא את הבקשות שנשלחו לחיים ועדיין בסטטוס "חדש"
const response = await api.get(`/Requests/professional/${profId}/pending`);
  ...
  const handleAcceptRequest = async (requestId: number) => {
    try {
      console.log('מנסה לאשר הזמנה:', requestId);
      
      // עדכון סטטוס ההזמנה ל'בטיפול'
      const response = await api.put(`/Requests/${requestId}/status`, { status: 'בטיפול' });
      console.log('תגובת השרת:', response);
      
      // הסרת ההזמנה מהרשימה הזמינה
      setAvailableRequests(prev => prev.filter((req: any) => req.id !== requestId));
      
      alert('ההזמנה אושרה בהצלחה!');
    } catch (error) {
      console.error('שגיאה באישור הזמנה:', error);
      alert('אירעה שגיאה באישור ההזמנה. אנא נסה שוב.');
    }
  };
